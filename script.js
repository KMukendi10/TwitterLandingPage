...
function wireLikeButton(btn) {
  btn.addEventListener('click', () => {
    ...
    iconEl.src = liked ? LIKE_FILLED_ICON : LIKE_ICON;
...
function wireBookmarkButton(btn) {
  btn.addEventListener('click', () => {
    ...
    const iconEl = btn.querySelector('img');
    iconEl.src = saved ? BOOKMARK_FILLED_ICON : BOOKMARK_ICON;
  });
}
...
// Builds a tweet using the exact same markup/classes as the
// existing tweets, so all current CSS applies unchanged, and
// wires up its like/retweet/bookmark/more-menu buttons the
// same way the original tweets are wired.
// ============================================================
